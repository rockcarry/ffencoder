...
static void add_stream(OUTSTREAM *ost, AVFormatContext *oc, AVCodec **codec, enum AVCodecID codec_id)
{
    AVCodecContext *c;
    int i;
...
static AVFrame *get_video_frame(OUTSTREAM *ost)
{
    AVCodecContext *c = ost->st->codec;
...
static int write_video_frame(AVFormatContext *oc, OUTSTREAM *ost)
{
    int ret;
    AVCodecContext  *c;
    AVFrame     *frame;
...
static AVFrame *get_audio_frame(OUTSTREAM *ost)
{
    ...
    for (j = 0; j <frame->nb_samples; j++) {
        for (i = 0; i < ost->st->codec->channels; i++) { *q++ = rand(); }
    }
 ...
 */
static int write_audio_frame(AVFormatContext *oc, OUTSTREAM *ost)
{
    AVCodecContext *c;
...
int main(void)
{
    ...
    if (fmt->video_codec != AV_CODEC_ID_NONE) {
        add_stream(&vstream, ctxt, &vcodec, fmt->video_codec);
        have_video   = 1;
        encode_video = 1;
    }
    if (fmt->audio_codec != AV_CODEC_ID_NONE) {
        add_stream(&astream, ctxt, &acodec, fmt->audio_codec);
        have_audio   = 1;
        encode_audio = 1;
    }
    ...
    while (encode_video || encode_audio) {
        ...
        if (encode_video &&
            (!encode_audio || av_compare_ts(vstream.next_pts, vstream.st->codec->time_base,
                                            astream.next_pts, astream.st->codec->time_base) <= 0)) {
            encode_video = !write_video_frame(ctxt, &vstream);
        } else {
    ...
    /* close each codec. */
    if (have_video) close_stream(ctxt, &vstream);
    if (have_audio) close_stream(ctxt, &astream);

    /* close the output file. */
    if (!(fmt->flags & AVFMT_NOFILE)) avio_close(ctxt->pb);
